...
module.exports = function(parent) {
    parent.call = function(endpoint, params = {}, data = false) {
        ...
        sig = parent._buildSignature(config.method.toUpperCase(), url, config.token);
        ...
        // If calling a protected method or not using patron level auth,
        // Append date & authorization to call header
        if(config.auth === 'protected' || config.authlvl !== 'patron') {
            cfg.headers.PolarisDate = sig.date;
            cfg.headers.Authorization = sig.sig;
        }
